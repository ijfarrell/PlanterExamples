                   ...
                   size_t                    size)
{
    ...
    float time_knob = euro.patch.GetAdcValue(CV_1);
    float time      = fmap(time_knob, 0.3f, 0.99f);

    float damp_knob = euro.patch.GetAdcValue(CV_2);
    float damp      = fmap(damp_knob, 1000.f, 19000.f, Mapping::LOG);

    float in_level = euro.patch.GetAdcValue(CV_3);

    float send_level = euro.patch.GetAdcValue(CV_4);
